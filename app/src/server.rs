...
use app::{
    build_cmd_response, create_cmd_user, Address, LogLevel, Node, ProxyBuilder, DNS_CACHE_TIMEOUT,
};
use app::{log_cleanup, log_init};
...
fn main() -> Result<()> {
    ...
    //                 Duration::from_secs(DNS_CACHE_TIMEOUT),
    //             )));
    ...
    CREATE TABLE IF NOT EXISTS users (
        ...
        // let cmd_task =
        spawn(async move {
            let socket = UdpSocket::bind("127.0.0.1:12771").await.unwrap();
            let mut buf = vec![0u8; 1024];
        ...
        let proxy =
            ProxyBuilder::new(local_addr, tls_acceptor.clone(), cleanup_abortable, cache).await;
        info!("Im here");
        proxy.start(1).await;
    });
